...
resource "dotfiles_file" "gitconfig" {
  ...
  strategy    = "copy"  # Will be removed
  file_mode   = "0644"
}

resource "dotfiles_file" "nvim_config" {
  repository  = dotfiles_repository.main.id
  name        = "neovim-config"
  source_path = "nvim/"
  target_path = "~/.config/nvim/"
  strategy    = "symlink"  # Will become dotfiles_symlink
}
...
resource "dotfiles_file" "fish_template" {
  ...
  strategy      = "template"  # Will become is_template=true
  file_mode     = "0644"
